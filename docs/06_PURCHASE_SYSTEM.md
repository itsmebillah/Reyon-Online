...
Sprint 21 accounting recognizes a financial supplier payable only when accepted inventory is received. Each accepted receipt posts its authoritative immutable net valuation as Debit Inventory / Credit Accounts Payable. Partial and multiple receipts post independently. A physically completed purchase return creates a separate Debit Accounts Payable / Credit Inventory supplier-credit journal using its immutable current-WAC return valuation. Operational supplier-payment status remains separate; payments do not reduce the accounting balance until the later Supplier Payment Accounting milestone.
