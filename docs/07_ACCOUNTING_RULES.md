...
The released Sprint 21 posting boundary creates immutable, private double-entry journals only from the existing `Completed` sale record. Each journal has a database-generated `JRN-YYYY-XXXXXX` reference, posting and source timestamps, source module and record, idempotency key, system/actor attribution, positive debit-or-credit lines, stored debit and credit totals, and an optional reversal reference. Database constraints and the controlled transaction require positive lines and equal totals; update and delete triggers protect headers and lines.

Completed-sale posting is configuration-gated and idempotent. It debits exactly one active Finance-approved payment or clearing account selected from the persisted payment kind and evidence state, debits the configured contra-revenue account for audited discounts, credits configured Product Sales, and separately credits configured Delivery Revenue when applicable. Ambiguous or inactive account mappings, unverified prepaid evidence, uncollected COD, inactive configuration, invalid values, or imbalance stop posting. A retry returns the existing source-linked journal. No account or monetary value is generated by the posting workflow.

The supplier-payable accounting milestone recognizes each accepted receipt at its existing authoritative net valuation: Debit Inventory Asset and Credit Accounts Payable. PO creation, approval, ordering, and unaccepted quantities create no payable. Partial and multiple receipts post independently. Purchase returns create append-only supplier credits at the authoritative current-WAC return valuation: Debit Accounts Payable and Credit Inventory. Supplier balances derive from immutable payable and credit events; operational supplier payments are not yet financial settlement postings.

Supplier Payment Accounting, refunds, expenses, reconciliation, and period close remain outside this milestone.
