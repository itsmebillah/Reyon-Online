...
| Media asset        | Provider-neutral reusable file identity, location, validation and licence evidence | Can be associated with multiple products without file copies  |
| Product media      | Ordered product-to-asset relationship and accessibility text                       | Belongs to a product; can identify one primary relationship   |
...
Brand, Category, guided Product creation, operational Product publication controls, and Product Media Management are released. Brand administration includes operational search/filter/sort, merchandising order and featured status, country, audit timestamps, case-insensitive duplicate-name prevention, and generated slugs; Products retain stable Brand foreign keys when presentation data changes. Product entry atomically creates the approved identity, primary category, first variant, price facts, website offer, and primary image relationship. Product files are reusable provider-neutral Media Assets, initially stored in Supabase Storage; ordered Product Media relationships carry product-specific ALT text and allow one physical asset to serve multiple products. Administrators can upload from their device or select the Media Library without entering storage URLs. Product galleries enforce the approved file, dimension, size, ALT-text, licensing, ordering, replacement, retention, and gallery-size controls without changing Product identity. Published records synchronize immediately through the customer projection. Private purchase prices, hidden/archived relationships, and non-published products remain excluded from customer reads.
