...
Brand, Category, guided Product creation, and operational Product publication controls are released. Brand administration includes operational search/filter/sort, merchandising order and featured status, country, audit timestamps, case-insensitive duplicate-name prevention, and generated slugs; Products retain stable Brand foreign keys when presentation data changes. Product entry atomically creates the approved identity, primary category, first variant, price facts, website offer, and primary image reference. Administrators can move records one approved step at a time through Review, Approved, Published, Hidden, and Archived with append-only evidence. Published records synchronize immediately through the customer projection. Private purchase prices, hidden/archived relationships, and non-published products remain excluded from customer reads. Product Media upload and Inventory Entry remain feature-blocked by their documented business decisions.
