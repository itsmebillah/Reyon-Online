...
begin
 ...
 if not exists(select 1 from catalog.products where slug='reyon-heritage-38-demo') then
  ...
  p:=public.admin_create_watch(jsonb_build_object('p_name','REYON Field 40 â€” Demo','p_slug','reyon-field-40-demo','p_brand_id',b,'p_category_id',c2,'p_variant_label','Steel / Sage nylon','p_sku','RYN-FLD-40-DEMO','p_purchase_price',3900,'p_selling_price',6290,'p_compare_at_price',6990,'p_asset_id',a,'p_image_alt','REYON Field everyday watch','p_country_code','BD','p_product_code','DEMO-FLD-40','p_publish',true,'specifications',jsonb_build_object('model','Field 40','gender','unisex','movement','Automatic','display','Analog','dialColor','Cream','caseColor','Silver','caseMaterial','Stainless steel','strapMaterial','Nylon','strapColor','Sage','caseSize','40 mm','waterResistance','10 ATM','warranty','Demo coverage â€” verify before sale'),'description','Demo catalog item for testing the REYON watch storefront.'));
  ...
  p:=public.admin_create_watch(jsonb_build_object('p_name','REYON Summit Chrono â€” Demo','p_slug','reyon-summit-chrono-demo','p_brand_id',b,'p_category_id',c3,'p_variant_label','Graphite / Black rubber','p_sku','RYN-SUM-CHR-DEMO','p_purchase_price',5200,'p_selling_price',8490,'p_compare_at_price',9490,'p_asset_id',a,'p_image_alt','REYON Summit sports chronograph','p_country_code','BD','p_product_code','DEMO-SUM-CHR','p_publish',true,'specifications',jsonb_build_object('model','Summit Chrono','gender','men','movement','Quartz','display','Chronograph','dialColor','Graphite','caseColor','Graphite','caseMaterial','Stainless steel','strapMaterial','Silicone','strapColor','Black','caseSize','42 mm','waterResistance','10 ATM','warranty','Demo coverage â€” verify before sale'),'description','Demo catalog item for testing the REYON watch storefront.'));
